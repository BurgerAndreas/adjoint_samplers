...
class VPSDE(BaseSDE):
    ...
    def sample_posterior(self, t, x0, x1):
        ...
        mu = (
            torch.exp(coeff1)
            * (1 - torch.exp(2 * coeff2))
            / (1 - torch.exp(2 * coeff3))
            * x0
            + torch.exp(coeff2)
            * (1 - torch.exp(2 * coeff1))
            / (1 - torch.exp(2 * coeff3))
            * x1
        )
...
class Graph:
    ...
    def is_freemean(self, x: torch.Tensor):
        return graph_utils.is_freemean(x, self.n_particles, self.spatial_dim)
...
class GraphVESDE(Graph, VESDE):
    ...
    def __init__(
        self, n_particles: int = 3, spatial_dim: int | None = None, *args, **kwargs
    ):
